...
export const register_event_sender = (_rust_app_ctx_ptr:number) => (_send_event_to_rust:RustEventSender) => {
    ...
    /*
    send_event([BridgeEvent.AddTodo, "bug with complete after remove+add"]);
    send_event([BridgeEvent.AddTodo, "clear completed"]);
    send_event([BridgeEvent.AddTodo, "select all"]);
    send_event([BridgeEvent.AddTodo, "local storage"]);
    send_event([BridgeEvent.RemoveTodo, "[2,0]"]);
    send_event([BridgeEvent.AddTodo, "Foo"]);
    */
}